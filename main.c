...
Bool is_top(const Window w) { return w == clients[clients_n - 1]; }

unsigned long get_idx(const Window w) {
    unsigned long i;
...
void add(const Window w) {
    ...
    focus(w);
    // Add window to list
    clients = realloc(clients, ++clients_n * sizeof(Window));
    clients[clients_n - 1] = w;
...
void remove(const Window w) {
    const unsigned long idx = get_idx(w);
    if (idx < clients_n)
        memmove(&clients[idx], &clients[idx + 1], (--clients_n - idx) * sizeof(Window));
}
...
void focus_in(const XFocusInEvent *e) {
    const Window w = clients[clients_n - 1];
    if (w != e->window)
        focus(w);
}
...
void close(void) { if (clients_n > 0) delete(clients[clients_n - 1]); }

void last(void) { if (clients_n > 1) pop(clients[clients_n - 2]); }
